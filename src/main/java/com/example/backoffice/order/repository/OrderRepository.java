...
public interface OrderRepository extends JpaRepository<Order, Long> {
    ...
    Page<Order> findByOrderNumberKeyword(String searchKeyword, Pageable pageable, OrderStatus status);

    @Query(value = "SELECT o FROM Order o " +
            "JOIN o.user u " +
            "WHERE (:keyword IS NULL OR u.name LIKE %:keyword%) " +
            "AND (:status IS NULL OR o.status = :status)")
    Page<Order> findByNameKeyword(String searchKeyword, Pageable pageable, OrderStatus status);
}
